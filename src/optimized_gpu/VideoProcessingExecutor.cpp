...
void VideoProcessingExecutor::processImages() {
    ...
    png_bytep *bg_pointers, *fg_pointers = nullptr;
    load_image(backgroundPath, &width, &height, &color_type, &bit_depth, &bg_pointers, true);
    ...
    for (auto it = imagePaths.begin() + 1; it != imagePaths.end(); ++it) {
        ...
        load_image(fgPath, &width, &height, &color_type, &bit_depth, &fg_pointers, true);
